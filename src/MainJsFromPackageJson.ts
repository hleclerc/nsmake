...
class MainJsFromPackageJson extends Task {
    exec( args: { js_env: string, typescript: boolean } ) {
        ...
        const fout = this.get_filtered_target( aout        , cwd, null, false ) || ( path.extname( aout ) == bext ? null :
                     this.get_filtered_target( aout + bext , cwd, null, false ) );

        if ( fout == null )
            throw `Impossible to find target '${ aout }' (mentionned in '${ name }' file)`;
        this.outputs = [ fout.name ];
    }
}
