...
public:
    struct CnData {
        void write_to_stream( std::ostream &os ) const { os << "pouet"; }
    ...
    NumAndSignature    get_first_filtered_target_signature( std::vector<std::string> targets, std::string cwd, bool allow_generation = true ); ///< get signature for generator of first possible `target`
